...
class CoverLetterSelector(Selector):
    def __init__(self, word_limit:int=600, temperature=0.0, model="gpt-3.5-turbo", option = "classic") -> None:
        super().__init__(temperature,model)
        self.word_limit = word_limit
        if option == "classic":
            self.cover_letter_style = "generate a cover letter as part of the job application. The cover letter should not contain any contact information (to or from) and only contain a body of 3 information dense paragraphs using business causal language."
        elif option == "modern":
